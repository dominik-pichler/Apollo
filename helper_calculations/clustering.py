...
def apply_kMeans_clustering_2D(raw_data: pd.DataFrame, label: str) -> dict:
	...
	while i <= max(cluster_km):
		...
		dict_regression_summary["regression_cluster_" + str(i)] = str(sm.OLS(np_raw_data_values[cluster_km == i, 1], sm.add_constant(np_raw_data_values[cluster_km == i, 0])).fit().summary())
		results_summary = sm.OLS(np_raw_data_values[cluster_km == i, 1], sm.add_constant(np_raw_data_values[cluster_km == i, 0])).fit().summary()
		results_as_html += "<h3 style='text-align:center;'> Cluster " + str(0) + "</h3>" + results_summary.tables[0].as_html() + "<br>" + results_summary.tables[1].as_html() + "<br>" + results_summary.tables[2].as_html() + "<br><br>"
	...
	label_k_means_clustering = "results/data/" + "k-Means_Clustering_for_" + label + "_" + datetime.today().strftime('%Y_%m_%d')
	with open(label_k_means_clustering + ".json", 'w') as fp:
