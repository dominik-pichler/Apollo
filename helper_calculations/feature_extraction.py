...
def apply_linear_feature_extraction(df_raw_data_FE: pd.DataFrame, label: str) -> dict:
	...
	label_PCA_Result_data = "results/data/" + "PCA_for_" + label + "_" + datetime.today().strftime('%Y_%m_%d')

	dict_results = {}
	dict_results["PCA_expl_var"] = list_explaind_vd
	dict_results["PCA_Data"] = df_principal.to_dict()

	with open(label_PCA_Result_data + ".json", 'w') as fp:
...
def apply_non_lin_feature_extraction(raw_data: pd.DataFrame, label: str,
	...
	label_tsne_Result_data = "results/data/" + "t-SNE_for_" + label + "_" + datetime.today().strftime('%Y_%m_%d')

	dict_results = {}
	dict_results["tSNE_Data"] = df_tsne_result.to_dict()
	with open(label_tsne_Result_data + ".json", 'w') as fp:
		json.dump(dict_results, fp)
